  ...
  <div class="admin-wrapper">
    ...
    <main class="content-area">
      <div class="operation-bar">
        <el-input v-model="movieName" placeholder="搜索电影..." clearable style="width: 300px; margin-right: 16px;"
      ...
      <el-table :data="movies" style="width: 100%; margin-top: 16px;" v-loading="loading" border>
        <el-table-column label="海报" width="165" align="center">
          <template #default="{ row }">
            <el-image v-if="row.movieImage === undefined" :src="`https://renmen321.cn:8080/api/images/${row.image}`"
        ...
        <el-table-column label="类型" width="200" align="center">
          <template #default="{ row }">
            <el-tag v-for="type in (Array.isArray(row.type) ? row.type : (row.type || '').split(' '))" :key="type"
              type="success" effect="plain" style="margin-right: 5px;">
              {{ type }}
            </el-tag>
          </template>
        </el-table-column>
      ...
      <div style="margin-top: 16px; float: right;">
        <el-pagination background layout="prev, pager, next, jumper" :total="total" :page-size="pageSize"
          v-model:current-page="pageNum" @current-change="handlePageChange" />
      </div>

    </main>

    <!-- 电影表单弹窗 -->
    <el-dialog v-model="dialogVisible" :title="dialogType === 'add' ? '新增电影' : '编辑电影'" width="50%" center>
      <el-form :model="currentMovie" :rules="rules" label-width="100px" ref="formRef">
        <el-form-item label="电影海报" prop="poster">
          <el-upload action="#" :show-file-list="false" :on-change="handleUpload" :auto-upload="false">
            <el-image v-if="currentMovie.movieImage === undefined"
              :src="`https://renmen321.cn:8080/api/images/${currentMovie.image}`" class="upload-poster" fit="cover"
        ...
        <el-form-item label="上映年份" prop="yearOfRelease">
          <el-date-picker v-model="currentMovie.yearOfRelease" type="year" value-format="YYYY" placeholder="请选择上映年份" />
        </el-form-item>
        ...
        <el-form-item label="电影类型" prop="type">
          <el-select v-model="currentMovie.type" multiple placeholder="请选择类型" style="width: 100%;">
        ...
        <el-form-item label="剧情简介" prop="introduction">
          <el-input v-model="currentMovie.introduction" type="textarea" :rows="4" maxlength="500" show-word-limit
            placeholder="请输入剧情简介" resize="none" />
        </el-form-item>
      </el-form>
